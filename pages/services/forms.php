...
<body>
  ...
  <main class="main">
    ...
    <div class="bottom-dashboard-section" style="padding-top: 0;">
       ...
       <div class="custom-pdf-container" id="forms-pdf-container" style="position: relative;">
           <div class="pdf-header">
               ...
               <div class="pdf-actions">
                   ...
                   <button class="floating-fullscreen-btn" onclick="expandPdf(this.dataset.currentUrl)" data-current-url="<?php echo htmlspecialchars($forms_docs[0]['file_path']); ?>" id="forms-fullscreen-btn" style="box-shadow: 0 4px 15px rgba(0,0,0,0.15); border-radius: 8px; background: #3b82f6; color: white; border: none; padding: 8px 16px; font-weight: 700; cursor: pointer; transition: transform 0.2s; display: flex; align-items: center; gap: 8px;">
                       <i class="fas fa-expand"></i> Open Document
                   </button>
               </div>
           </div>
           
           <div class="pdf-body" style="cursor: pointer; position: relative; width: 100%; aspect-ratio: 1 / 1.414; background: #fff; overflow: hidden; border-radius: 0 0 12px 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.08); border-top: none;" onclick="document.getElementById('forms-fullscreen-btn').click()" title="Click to open Document">
               <canvas id="forms-pdf-canvas" data-pdf-url="<?php echo htmlspecialchars($forms_docs[0]['file_path']); ?>" style="width: 100%; height: 100%; object-fit: cover; opacity: 0; transition: opacity 0.4s;"></canvas>
               <div id="forms-pdf-loader" style="position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #f8fafc; color: #3b82f6;">
  ...
  <script>
    ...
    function loadPdf(pdfUrl, title, btnElement) {
        ...
        const titleText = document.getElementById('forms-title-text');
        if(titleText) titleText.innerText = title;
        
        const fullscreenBtn = document.getElementById('forms-fullscreen-btn');
        if (fullscreenBtn) {
            fullscreenBtn.dataset.currentUrl = pdfUrl;
        }

        renderThumbnail('forms-pdf-canvas', pdfUrl, 'forms-pdf-loader');
    }
    ...
    window.addEventListener('DOMContentLoaded', () => {
        const canvas = document.getElementById('forms-pdf-canvas');
        if(canvas && canvas.dataset.pdfUrl) {
            renderThumbnail('forms-pdf-canvas', canvas.dataset.pdfUrl, 'forms-pdf-loader');
        }
    });

...
$internship_docs = [];
try {
    $stmt = $pdo->query("SELECT * FROM internship_documents ORDER BY id DESC");
    $internship_docs = $stmt->fetchAll();
...
<body>
  ...
  <main class="main">
    <div class="bottom-dashboard-section" style="padding-top: 20px;">
       ...
       <div class="doc-buttons">
           <?php foreach ($internship_docs as $index => $doc): ?>
               <button class="doc-btn <?php echo $index === 0 ? 'active' : ''; ?>" onclick="loadPdf('<?php echo htmlspecialchars($doc['file_path']); ?>', this)">
                   <i class="fas fa-briefcase"></i> <?php echo htmlspecialchars($doc['title']); ?>
               </button>
           <?php endforeach; ?>
       </div>

       <?php if (count($internship_docs) > 0): ?>
       <!-- Interactive PDF Viewer -->
       <div class="custom-pdf-container" id="internship-pdf-container">
           <div class="pdf-header">
               <div class="pdf-title">
                   <i class="fas fa-file-pdf"></i> <span id="pdf-title-text">Official Internship Document</span>
               </div>
               <div class="pdf-actions">
                   <a href="<?php echo htmlspecialchars($internship_docs[0]['file_path']); ?>" id="pdf-download-btn" download class="pdf-btn pdf-btn-outline">
       ...
       <div class="empty-state-container" style="background: white; padding: 40px; border-radius: 12px; text-align: center; border: 1px solid var(--border-color);">
           ...
           <div class="empty-state-desc" style="font-size: 14px; color: var(--text-muted); margin-bottom: 24px;">The internship guidelines have not been uploaded by the administration yet.</div>
       </div>
       <?php endif; ?>
  ...
  <script>
    function loadPdf(pdfUrl, btnElement) {
        ...
        // Update iframe
        const iframe = document.getElementById('internship-pdf');
        ...
        const container = document.getElementById('internship-pdf-container');
